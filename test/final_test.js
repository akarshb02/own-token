...
contract('D_Token', function(accounts) {
    ...
    it('handles delegated token transfer', function() {
        return dapp.deployed().then(function(instance) {
            ...
            assert(error.message.indexOf('revert') >= 0, 'cannot transfer larger then approved amt');
            return tokenInstance.transferFrom.call(fromAccount, toAccount, 10, { from: spendingAccount });
        }).then(function(success) {
            assert.equal(success, true);
            return tokenInstance.balanceOf(fromAccount);
        }).then(function(balance) {
            assert.equal(balance.toNumber(), 100, 'detucts the amt from the sending accounnt');
            return tokenInstance.balanceOf(toAccount);
        }).then(function(balance) {
            assert.equal(balance.toNumber(), 0, 'adds the amt from receiver account');
            return tokenInstance.allowence(fromAccount, spendingAccount);
        }).then(function(allowence) {
            assert.equal(allowence.toNumber(), 10, "detucts the amt from allowence");
